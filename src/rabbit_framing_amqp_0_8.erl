...
-type(amqp_method_field_name() ::
       ( identifier | insist | nowait | prefetch_count
       | mandatory | consumer_tag | message_count | arguments
       | exclusive | delivery_tag | host | routing_key
       | string_2 | cluster_id | ticket | content_checksum
       | channel_max | routing_key | integer_result | exclusive
       | response | requeue | consumer_tag | version_minor
       | write | exchange | reply_text | reply_code
       | exchange | ticket | details | queue
       | routing_key | meta_data | result | result
       | routing_key | no_local | ticket | multiple
       | queue | capabilities | reply_code | virtual_host
       | known_hosts | string_result | delivery_tag | routing_key
       | auto_delete | exchange | dtx_identifier | consume_rate
       | immediate | locale | internal | consumer_tag
       | exchange | server_properties | nowait | reply_text
       | active | operation | active | global
       | reply_text | no_ack | immediate | nowait
       | nowait | queue | delivery_tag | passive
       | prefetch_count | ticket | if_unused | exchange
       | consumer_tag | heartbeat | method_id | consumer_tag
       | consumer_tag | ticket | queue | passive
       | queue | class_id | frame_max | ticket
       | consumer_tag | reply_text | exchange | mechanisms
       | read | durable | nowait | requeue
       | exchange | out_of_band | reply_code | ticket
       | delivery_tag | durable | no_local | immediate
       | reply_code | nowait | nowait | exchange
       | delivery_tag | nowait | requeue | challenge
       | consumer_tag | locales | consumer_tag | staged_size
       | multiple | ticket | ticket | no_ack
       | ticket | routing_key | routing_key | ticket
       | routing_key | arguments | exclusive | string_1
       | mandatory | consumer_count | string_op | integer_1
       | queue | reply_text | exclusive | table
       | delivery_tag | message_count | requeue | queue
       | exchange | exchange | arguments | if_empty
       | method_id | identifier | exchange | identifier
       | consumer_tag | ticket | prefetch_size | auto_delete
       | exchange | queue | global | integer_3
       | nowait | reply_text | routing_key | redelivered
       | delivery_tag | no_local | exclusive | redelivered
       | nowait | class_id | delivery_tag | known_hosts
       | queue | integer_4 | consumer_tag | frame_max
       | prefetch_size | message_count | message_count | type
       | mechanism | global | client_properties | no_ack
       | integer_2 | mandatory | nowait | consumer_tag
       | integer_op | arguments | consumer_tag | prefetch_size
       | consumer_tag | routing_key | response | routing_key
       | content_size | ticket | active | operation
       | nowait | reply_code | reply_code | prefetch_count
       | queue | ticket | heartbeat | channel_max
       | ticket | exchange | passive | exchange
       | if_unused | queue | realm | version_major
       | consumer_tag | redelivered )).
